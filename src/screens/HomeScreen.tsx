...
function estaAberto(horario?: string, diasFuncionamento?: string[]): boolean {
  ...
  const toMin = (h: string) => {
    ...
    return parseInt(hr, 10) * 60 + (mn ? parseInt(mn, 10) : 0);
  };
...
function VerificadosSection({
  ...
  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
    ...
    <Text style={sv.titulo}>Espaços em destaque</Text>
  </View>

  <Text style={sv.subtitulo}>
        ...
        {verificados.map((_, idx) => (
          <View
            ...
            style={[
              sv.dot,
              {
                backgroundColor: idx === autoIdx ? GOLD : '#333',
              },
            ]}
          />
        ))}
...
export default function HomeScreen() {
  ...
  return (
    <View style={s.container}>
      ...
      <LinearGradient colors={['#000000', '#1A1A1A']} style={s.header}>

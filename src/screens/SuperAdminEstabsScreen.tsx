...
export default function SuperAdminEstabsScreen() {
  ...
  return buscaOk && (
    ...
    if (filtro === 'pagantes') {
      return buscaOk && e.assinaturaAtiva && e.plano !== 'free' && e.plano !== 'trial';
    }
            ...
            try {
              ...
              await fn({
                ...
                motivo: aprovado
                  ...
                  : 'Não atende os critérios necessários',
              });

              Alert.alert(
                '✅',
                aprovado
                  ? 'Selo aprovado com sucesso!'
      ...
      <View style={s.filtrosWrap}>
        {FILTROS.map(f => (
          <TouchableOpacity
            ...
            style={[
              s.filtroChip,
              filtro === f.k && s.filtroChipAtivo,
            ]}
          >
            <Text
              style={[
                s.filtroText,
                filtro === f.k && s.filtroTextAtivo,
              ]}
            >
              {f.l}
            </Text>
            ...
            <View style={s.seloCard}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View
                    style={[
                      s.planoBadge,
                      {
                        backgroundColor: corPlano(item.plano || 'free') + '22',
                  ...
                  >
                    <Text
                      style={[
                        s.planoBadgeText,
                        { color: corPlano(item.plano || 'free') },
                      ]}
                    >
              ...
              <View style={s.seloAcoes}>
                <TouchableOpacity
                  style={s.btnAprovar}
                  onPress={() =>
                    responderSelo(item.id, true, item.estabelecimentoNome)
                  }
                >
                ...
                <TouchableOpacity
                  style={s.btnRejeitar}
                  onPress={() =>
                    responderSelo(item.id, false, item.estabelecimentoNome)
                  }
                >
              ...
              <View style={s.cardTop}>
                <View style={{ flex: 1 }}>
                  <View style={s.cardNomeRow}>
                    ...
                    {(item.verificado || item.plano === 'elite') && (
  ...
  <Text style={s.verificadoBadge}>
    {destaquePagoValido(item) ? '⭐' : '👑'}
                  ...
                  <Text style={s.cardMeta}>
                    {item.tipo} • {item.cidade || 'Sem cidade'}
                  </Text>
                  ...
                  {item.solicitacaoSeloStatus === 'pendente' && (
                    <View style={s.seloSolicitadoBadge}>
                      <Text style={s.seloSolicitadoText}>
                        🔔 Selo solicitado
                      </Text>
                    </View>
                  )}
              ...
              <View style={s.togglesRow}>
                <View style={s.toggleItem}>
                  ...
                  <Switch
                    ...
                    onValueChange={() =>
                      toggleAtivo(item.id, item.ativo || false)
                    }
                    trackColor={{
                      false: '#333',
                      true: GOLD + '55',
